...
public class ApiTestRestAssured {
	...
	public static void setup() throws Exception {
		...
		TruckRequest truckRequest = new TruckRequest(TruckConstants.TRANSPORTER_ID,
		...
		TruckRequest truckRequest1 = new TruckRequest(TruckConstants.TRANSPORTER_ID,
				"AB 00 AA 1113","alpha", 50, "driver:0de885e0-5f43-4c68-8dde-b25464747865", 20, (long) 40,
				TruckData.TruckType.OPEN_HALF_BODY);
	...
	@Test
	@Order(1)
	public void addDataFailed_nullTransporterId() throws Exception {
		...
		assertEquals("[{field=transporterId, message=Transporter Id can not be null, rejectedValue=null, object=truckRequest}]",
				jsonPathEvaluator.get("apierror.subErrors").toString());
		}
	
	
	@Test
	@Order(1)
	public void addDataFailed_invalidTyres() throws Exception {

		TruckRequest truckRequest = new TruckRequest("transporterId:129", "AZ 00 AA 1111", "alpha", 50,
				"driver:0de885e0-5f43-4c68-8dde-b25464747865", 3, null, TruckData.TruckType.OPEN_HALF_BODY);
		...
		assertEquals("UNPROCESSABLE_ENTITY", jsonPathEvaluator.get("apierror.status").toString());
		assertEquals("Error: Failed: invalid number of tyres", jsonPathEvaluator.get("apierror.message").toString());
		}
	...
	@Test
	@Order(1)
	public void addData_sameTruckNo_differentTransporterId() throws Exception {

		TruckRequest truckRequest = new TruckRequest("transporterId:124", "AA 00 AA 1111", "alpha", 50,
				"driver:0de885e0-5f43-4c68-8dde-b25464747865", 20, null, TruckData.TruckType.OPEN_HALF_BODY);
	...
	public void addDataFailed_truckNoAlreadyExisted() throws Exception {
		...
		assertEquals("CONFLICT", jsonPathEvaluator.get("apierror.status").toString());	
		assertEquals("Truck number already exists Enter a different truck number", jsonPathEvaluator.get("apierror.message").toString());

	}

	@Test
	@Order(3)
	public void addDataFailed_TruckNo_null() throws Exception {
		...
		assertEquals(400, response.statusCode());
		assertEquals("Validation error", jsonPathEvaluator.get("apierror.message").toString());
		assertEquals("[{field=truckNo, message=Truck Number can not be null, rejectedValue=null, object=truckRequest}]",
				jsonPathEvaluator.get("apierror.subErrors").toString());
		}
	...
	public void addDataFailed_invalidTruckNo_notNull1() throws Exception {
		...
		assertEquals(422, response.statusCode());
		assertEquals("UNPROCESSABLE_ENTITY", jsonPathEvaluator.get("apierror.status").toString());	
		assertEquals("Error: Failed: Enter Correct Truck Number", jsonPathEvaluator.get("apierror.message").toString());
	...
	public void getTruckDataWithIdSuccess() throws Exception {
		...
		assertEquals("transporterId:0de885e0-5f43-4c68-8dde-b0f9ff81cb67",
	...
	public void getTruckDataWithIdFailed() throws Exception {
		...
		assertEquals("NOT_FOUND", jsonPathEvaluator.get("apierror.status").toString());	
		assertEquals("TruckData was not found for parameters {truckId=truck:132536}", jsonPathEvaluator.get("apierror.message").toString());
	...
	public void getTruckDataWithParam_transporterId_truckApproved_True() throws Exception {
		...
		System.err.println(response.jsonPath());
	...
	@Test
	@Order(18)
	public void updateDataFailed_invalidTyres() throws Exception {

		TruckUpdateRequest truckUpdateRequest = new TruckUpdateRequest(null, null, 0, null, 3, null,
				TruckData.TruckType.OPEN_FULL_BODY);
	...
	public  void deleteDatafail() throws Exception{
		...
		String truckid = "truck:0a5f1700-041a-43d4-b3eb-00000000123";
		Response response1 = RestAssured.given().header("", "").delete("/" + truckid).then().extract().response();

		JsonPath jsonPathEvaluator = response1.jsonPath();

		assertEquals(404, response1.statusCode());
		assertEquals("TruckData was not found for parameters {truckId=truck:0a5f1700-041a-43d4-b3eb-00000000123}", jsonPathEvaluator.get("apierror.message").toString());
	}
